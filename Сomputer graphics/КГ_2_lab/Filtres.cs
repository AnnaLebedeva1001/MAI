...
namespace КГ_2_lab
{
    ...
    class Filtres
    {
        ...
        public static UInt32[,] matrix_filtration(int W, int H, UInt32[,] pixel, int N, double[,] matrix_)
        {
            ...
            Channel ColorOfPixel = new Channel();
            Channel ColorOfCell = new Channel();
            for (i = gap; i < tmpH - gap; i++)
                for (j = gap; j < tmpW - gap; j++)
                {
                    ColorOfPixel.R = 0;
                    ...
                    for (k = 0; k < N; k++)
                        for (m = 0; m < N; m++)
                        {
                            ColorOfCell = calculationOfColor(tmppixel[i - gap + k, j - gap + m], matrix_[k, m]);
        ...
        public static Channel calculationOfColor(UInt32 pixel, double coefficient)
        {
            Channel Color = new Channel();
        ...
        //сборка каналов
        public static UInt32 build(Channel ColorOfPixel)
        {
            UInt32 Color;
        ...
        public static double[,] laplas = new double[N1, N1] {{-1, -1, -1},
                                                               {-1,  8, -1},
                                                               {-1, -1, -1}};

        //фильтр Собеля
        public const int N2 = 3;
        public static double[,] sobel = new double[N1, N1] {{-1, 0, 1},
                                                           {-2, 0, 2},
                                                           {-1, 0, 1}};
    }
}
